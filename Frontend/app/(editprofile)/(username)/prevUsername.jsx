...
const PrevUsernameScreen = ({ navigation }) => {
  ...
  const handleNext = async () => {
    ...
    const requestBody = {
      ...
      use: "Username",
      text: `Hello ${username},\n\nYour OTP for Username Change is: ${otp}\n\nThank you,\nCityforge Team`
    };

    try {
      const response = await axios.post('http://192.168.210.178:8080/api/v1/email/send-otp', requestBody);

      if (response.status === 200) {
        console.log(`OTP sent to ${userData.email}: ${otp}`);
        navigation.navigate('VerificationUsernameScreen', { otp, username });
      } else {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.text}>Please enter your registered email ID & Username:</Text>
      <TextInput
        style={styles.input}
...
export default PrevUsernameScreen;
